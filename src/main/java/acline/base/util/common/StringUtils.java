...
public class StringUtils {
    ...
    public static boolean isIpSubnet(String str) {
        String[] sp = split(str, '/');
        if (sp.length == 2) {
            if (isIp(sp[0])) {
                try {
                    int i = Integer.valueOf(sp[1]);
                    if (i > 0 && i <= 32) {
                        return true;
                    }
                } catch (Exception e) {
    ...
    public static boolean isIp(String str) {
        String rexp = "\\b((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\.((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\.((?!\\d\\d\\d)"
     ...
     *
     * <pre>
     * StringUtils.formatMessage("Hi {}. My name is {}.", "Alice", "Bob");
     * </pre>
     *
     ...
     */
    public static String formatMessage(final String messagePattern, final Object... argArray) {
